...
World.prototype.turn = function() {
    ...
    if (this.lastReload && ((new Date()) - this.lastReload) > 5000 ) {
        ...
        var plantEaters = this.grid.getAllCritters([PlantEater, SmartPlantEater]);
        if (plants.length == 0 || plantEaters.length == 0) {
            this.reload();
        }
    }
};
...
SmartPlantEater.prototype.rememberDir = function(dir, type) {
    var ndir = directions[dir];
    var npos = new Vector(this.location.x, this.location.y);
    npos = npos.plus(ndir);
    if (this.map.length == 0) {
        this.map[this.location.x] = [];
        this.map[this.location.x][this.location.y] = null;
    }
    if (this.checkDirIsUnknown(npos)) {
        this.map[npos.x] = [];
    }
    this.map[npos.x][npos.y] = type;
};
SmartPlantEater.prototype.getUnknownDirs = function(avSpaces) {
    var unknowns = avSpaces.filter(function(space){
